...
public class IndexAdmin {
	...
	private JMenuItem BulkGoodsAdd;
	...
	private void init(String name) {
		...
		//设置框架大小及位置
		index.setBounds(200, 100, 600, 800);
	...
	public void indexadmin() {
		...
		goodsSelect=new JMenuItem("商品查询");
		goodsSelect.setFont(f);
		BulkGoodsAdd=new JMenuItem("批量添加");
		BulkGoodsAdd.setFont(f);
	...
	private void action(final String name) {
		...
		goodsSelect.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				//界面转换
				index.setVisible(false);
				new GoodsSelectAd(name);
			}
		});

		BulkGoodsAdd.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				index.setVisible(false);
				new BulkGoodsadd(name);
			}
		});
